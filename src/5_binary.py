...
if __name__ == '__main__':
    ...
    print(ipart)
    suffix = 'binary' #sys.argv[2]
    model = 'combined' #sys.argv[3] # 'combined' or 'cubic'
    sample = 'eb' #sys.argv[2] # ms or rg or cl # ind
    Nstar = 256 #64
    offset = 0
    ...
    stars = pd.read_csv(work_dir+'test_samples/samples_eb.csv')

    kics = np.loadtxt('exists.txt', dtype='int')
    idx = (np.isfinite(stars['Dnu'])) & (stars['[M/H]'] > -0.7) & (stars['numax'] < 230.) #& (~np.isin(stars['KIC'], kics)) 
    ...
    cols = ['KIC', 'Teff', 'e_Teff', '[M/H]', 'e_[M/H]', 'luminosity', 'e_luminosity', 
        'Dnu', 'e_Dnu', 'numax', 'e_numax', 'star_mass', 'e_star_mass', 'radius', 'e_radius', 'mod_e_freq']
    stars.loc[:,cols].to_csv(work_dir+'inputs/samples_{:0.0f}.csv'.format(random_index), index=False)

    modes = pd.read_csv(work_dir+'test_samples/modes_eb.csv')
    ...
    new_params = {
    "observables": ['star_mass', 'radius', '[M/H]'] , #' Teff', '[M/H]', 
    ...
    "estimators_to_plot": ['star_age', 'star_mass', 'radius', 'amlt'],
