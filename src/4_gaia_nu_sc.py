...
from grid import grid, get_model_Dnu, user_setup_params
...
if __name__ == '__main__':
    ...
    print(ipart)
    suffix = 'gaia_nu_sc' #sys.argv[2]
    model = 'combined' #sys.argv[3] # 'combined' or 'cubic'
    sample = sys.argv[2] # ms or rg or eb or cl # ind
    Nstar = 256 #64
    offset = 0
    ...
    if sample == 'ms':
        ...
        stars = pd.read_csv(work_dir+'../sample/samples_rg.csv')
    elif sample == 'eb':
        stars = pd.read_csv(work_dir+'../sample/samples_eb.csv')
    else:
        stars = pd.read_excel(work_dir+'../sample/samples_cl.xlsx')
    # kics = np.loadtxt('exists.txt', dtype='int')
    idx = (np.isfinite(stars['Dnu'])) & (np.isfinite(stars['luminosity'])) & \
        (stars['[M/H]'] > -0.7) & (stars['[M/H]'] < 0.6) & (stars['numax'] < 230.) # & (~np.isin(stars['KIC'], kics)) 
    ...
    if sample == 'ms':
        ...
        modes = pd.read_csv(work_dir+'../sample/modes_eb.csv')
        idx = (np.isin(modes['KIC'], stars['KIC'])) & (modes['l']<=2) # & 
    else:
        modes = pd.read_csv(work_dir+'../sample/modes_rg.csv')
    ...
    lists = []
    tracks = [work_dir+'../models/calibrated/'+f for f in os.listdir(work_dir+'../models/calibrated/') if (f.endswith('.npy') )]
