...
! ***********************************************************************
      ...
      integer function extras_check_model(id)
         ...
         ! protostar to rgb Dnu=2
         if ((s% center_h1 < 0.0001) .and. (s%delta_nu <= 1.5)) then
        ...
        ! if ((s%helium_ignition) .and. ((s%center_he4+s%center_he3) <0.95) ) then !(phase_helium_burning >=5)
        !    extras_check_model = terminate
        ...
        !  ! HeZAMS to AGB
        !  if ( (s%helium_ignition) .and. (s%center_he4+s%center_he3) < 0.001) then
        !     extras_check_model = terminate
      ...
      integer function extras_finish_step(id)
         ...
         ! ZAMS to RGB (numax = 300 muHz)
         if (((s%center_h1 < 0.0001) .or. &
               ((s%L_nuc_burn_total/s%L_surf)>=0.99) ) .and. (s%delta_nu>=1.5) ) then
            ! save profile models
            s% write_profiles_flag = .true. 

            ! use a custom frequency, save 5 profiles per 1muHz in Dnu, 
            ! or 0.2 profiles per 1K in Teff, or 100 profiles per 1dex in log10(L/Lsun)
            ! d(N_m)/d(Dnu) = d(N_m)/d(N_p) * d(N_p)/d(Dnu) = profile_interval * 5/1muHz
            ! d(N_m)/d(Teff) = d(N_m)/d(N_p) * d(N_p)/d(Teff) = profile_interval * 0.2/1K
            ! d(N_m)/d(log10(L)) = d(N_m)/d(N_p) * d(N_p)/d(log10(L)) = profile_interval * 100/1dex
            s% profile_interval = max( min(int(1./abs(s%xtra(1)-s%xtra_old(1))/5.), &
                                           int(1./abs(s%xtra(2)-s%xtra_old(2))/0.2), &
